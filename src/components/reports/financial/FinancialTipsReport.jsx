...
import { ClipboardList, DollarSign, Receipt, Wallet } from "lucide-react";
import {
  Table,
  TableBody,
  TableCell,
...
export default function FinancialTipsReport() {
  ...
  return (
    <FinancialPageHeader
      ...
      emptyMessage="No paid orders found for the selected period."
    >
      {data && !data.empty ? (
                ...
                {(data.byEmployee || []).length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={5} className="text-sm text-zinc-500">
                      No employee tip or charge activity.
                    </TableCell>
                  </TableRow>
                ) : (
