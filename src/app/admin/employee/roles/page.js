...
import { ArrowLeft, Loader2, Shield, Plus, Trash2 } from "lucide-react";
...
export default function EmployeeRolesPage() {
  ...
  const handleDeleteRole = async (id) => {
    ...
    try {
      const res = await fetch(`/api/employees/roles?id=${id}`, {
        method: "DELETE",
      });
      const json = await res.json();
      if (json.success) {
          ...
          <div className="flex flex-col sm:flex-row justify-between sm:items-end gap-4 border-b border-zinc-200 pb-5">
            <div>
              ...
              <p className="text-[15px] mt-1" style={{ color: PALETTE.inkMuted }}>
                Manage access levels and configure role-based permissions.
              </p>
            </div>
                  ...
                  {loading ? (
                    ...
                    <Table>
                      <TableHeader className="bg-zinc-50">
                        <TableRow>
                          <TableHead className="text-[12px] font-bold uppercase tracking-wider text-zinc-500 py-4 px-6">Role Name</TableHead>
                          <TableHead className="text-[12px] font-bold uppercase tracking-wider text-zinc-500 py-4 px-6">Permissions</TableHead>
                                ...
                                <div className="flex flex-wrap gap-2">
                                  {role.permissions && role.permissions.length > 0 ? (
                                    role.permissions.map((p, idx) => (
                                      <span key={idx} className="inline-flex bg-zinc-100 text-zinc-700 px-2 py-0.5 rounded text-[12px] font-medium border border-zinc-200">
                                        {p}
                                      </span>
                                    ))
                                  ) : (
                                    <span className="text-[13px] text-zinc-400">No specific permissions set</span>
                                  )}
                                </div>
                              </TableCell>
