...
import { ArrowLeft, Loader2, Calendar, Smartphone } from "lucide-react";
          ...
          <div className="flex flex-col sm:flex-row justify-between sm:items-end gap-4 border-b border-zinc-200 pb-5">
            <div>
              ...
              <p className="text-[15px] mt-1" style={{ color: PALETTE.inkMuted }}>
                View POS and Tablet devices assigned to employee shifts.
              </p>
            </div>
              ...
              <Card className="shadow-sm border-zinc-200 bg-white overflow-hidden h-full flex flex-col">
                <CardHeader className="bg-zinc-50/50 border-b border-zinc-100 pb-4 flex flex-row items-center justify-between gap-4">
                  <CardTitle className="text-[18px] font-bold text-zinc-900 flex items-center gap-2">
                    <Smartphone className="w-5 h-5 text-[#1e40af]" /> Shift Device Assignments
                  </CardTitle>
                </CardHeader>
                    ...
                    <Table>
                      <TableHeader className="bg-zinc-50">
                        <TableRow>
                          ...
                          <TableHead className="text-[12px] font-bold uppercase tracking-wider text-zinc-500 py-4 px-6">Assigned Device</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {shifts.length === 0 ? (
                          <TableRow>
                            <TableCell colSpan={3} className="h-32 text-center text-zinc-400 font-medium text-[14px]">
                              No shift device assignments found.
                            </TableCell>
                          </TableRow>
                        ) : (
                          shifts.map((shift) => (
                            <TableRow key={shift._id} className="h-16 hover:bg-zinc-50 transition-colors">
                              ...
                              <TableCell className="px-6 py-3">
                                {shift.assignedDevice ? (
                                  <span className="inline-flex bg-emerald-50 text-emerald-700 px-3 py-1 rounded text-[13px] font-bold border border-emerald-200">
                                    {shift.assignedDevice.deviceName} ({shift.assignedDevice.deviceType})
                                  </span>
                                ) : (
                                  <span className="text-[13px] text-zinc-400 italic">No device assigned</span>
                                )}
                              </TableCell>
                            </TableRow>
                          ))
