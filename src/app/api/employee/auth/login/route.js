...
async function resolveDeviceForPasswordLogin() {
...
async function finishEmployeeLogin({
  employee,
  device,
  browserFingerprint,
  ip,
...
export async function POST(request) {
  try {
    ...
    if (passcode) {
      const perPasscode = checkRateLimit({
        key: `employee-login-passcode:${ip}`,
        limit: 10,
        windowMs: 15 * 60_000,
      });
      if (!perPasscode.ok) {
        return rateLimitResponse(perPasscode.retryAfterSec, 'Too many login attempts. Please try again later.');
      }
      ...
      if (!ctx?.device) {
        return NextResponse.json({
          success: false,
          message: 'Passcode login is only available on a registered device.',
        }, { status: 403 });
      }
      ...
      const deviceResult = await resolveDeviceForPasswordLogin();
