/**
 * Soft reconciliation: payments should ≈ net + tax + tips − refunds ± rounding.
 */
...
export function reconcileEod(report) {
  ...
  const expected = r2(net + tax + tips - refunds);
  ...
  if (!ok) {
    messages.push(
      `Payment totals require attention before relying on this report. Expected ≈ ${expected.toFixed(2)} (net + tax + tips − refunds), actual payments ${actual.toFixed(2)} (difference ${difference.toFixed(2)}).`
    );
  }
